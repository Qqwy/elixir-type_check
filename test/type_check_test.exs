...
defmodule TypeCheckTest do
  ...
  describe "apply" do
    test "ok" do
      ...
      assert {:ok, 13} = TypeCheck.apply(type, Kernel, :abs, [-13])
    end

    test "wrong param type" do
      alias TypeCheck.Builtin, as: B
      type = B.function([B.atom()], B.number())
      assert {:error, _} = TypeCheck.apply(type, Kernel, :abs, [-13])
    end
    ...
    test "union of functions" do
      ...
      assert {:ok, 13} = TypeCheck.apply(type, Function, :identity, [13])
      assert {:ok, :hi} = TypeCheck.apply(type, Function, :identity, [:hi])
      assert {:error, _} = TypeCheck.apply(type, Function, :identity, ["wow"])
    end
  end
end
